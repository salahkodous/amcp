...
- **Errors:** `{error: {code, message, retryable, doc}}`. Registered codes: `unknown_agent`, `unknown_capability`, `capability_denied`, `terms_rejected`, `negotiation_failed`, `budget_exceeded`, `approval_timeout`, `artifact_rejected`, `settlement_failed`, `rate_limited`, `idempotency_replayed`, `idempotency_key_in_use` (same key, different payload — send a new key). Never silent degradation.
